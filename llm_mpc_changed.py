...
# F1TENTH gym — uses old OpenAI gym API, not gymnasium
import gym
import f110_gym  # registers f110-v0 with old gym
...
class RaceLLMMPC():
    def __init__(self,
        ...
        self.obs, _, _, _ = self.env.reset(poses=np.array([[0.0, 0.0, np.pi / 2]]))
    ...
    def _step_f1tenth(self, mpc_params: dict) -> tuple:
        ...
        self.obs, reward, self.f1tenth_done, info = self.env.step(action)
