...
class RaceLLMMPC():
    def __init__(self,
        ...
        reset_out = self.env.reset(poses=np.array([[0.0, 0.0, np.pi / 2]]))
    ...
    def _step_f1tenth(self, mpc_params: dict, append_history: bool = True) -> tuple:
        ...
        self.obs, reward, done, info = self.env.step(action) #self.obs, reward, terminated, truncated, info = self.env.step(action)
        self.f1tenth_done = bool(done) #self.f1tenth_done = bool(terminated or truncated)
        self._maybe_render()
