...
# Wrapped in try-except to handle network failures gracefully (e.g., on HPRC headless clusters)
try:
    ...
    print(f"[WARN] Failed to load embeddings (network issue): {type(e).__name__}")
    print(f"       RAG will be disabled. Model will still work without semantic search.")
    _LOCAL_EMBEDDINGS = None


# ============================================================================
# Output Parsing Utilities (Improved parser for LLM outputs with comments)
# ============================================================================

def remove_comments_from_dict(dict_str: str) -> str:
    """
    Remove inline comments from Python dictionary string while preserving string contents.
    
    Handles:
    - Comments after dictionary values: {'key': value, # comment
    - Multi-line dictionaries with comments on each line
    - Preserves # characters inside quoted strings
    
    Args:
        dict_str: String representation of a Python dictionary
        
    Returns:
        Dictionary string with comments removed
    """
    dict_lines = dict_str.split('\n')
    dict_lines_clean = []
    
    for line in dict_lines:
        # Remove comment part (text after #) but keep strings intact
        in_string = False
        quote_char = None
        cleaned_line = ""
        
        for i, char in enumerate(line):
            # Track string boundaries (simple approach: single/double quotes)
            ...
            # Stop at comment marker if not in string
            if char == '#' and not in_string:
                break
            
            cleaned_line += char
        
        dict_lines_clean.append(cleaned_line)
    
    return '\n'.join(dict_lines_clean)


def parse_mpc_output(text: str) -> tuple:
    """
    Extract MPC parameters from LLM output.
    
    Supports multiple formats:
    1. XML-wrapped (trained model output):
       <reasoning>...</reasoning>
       <answer>
       new_mpc_params = {'key': value, ...}
       </answer>
    
    2. Plain dictionary:
       new_mpc_params = {'key': value, ...}
    
    3. Dictionaries with comments (common in LLM outputs):
       new_mpc_params = {
    ...
    Returns:
        Tuple of (parsed_dict, explanation_text)
        - parsed_dict: Extracted dictionary or None if parsing failed
        - explanation_text: Original text (for logging/debugging)
    """
    command_dict = None
    ...
    try:
        ...
        if match:
            ...
            try:
                # Remove comments from dictionary (LLM may add explanatory comments)
                dict_str_no_comments = remove_comments_from_dict(dict_str)
                
                # Normalize whitespace (collapse multiple spaces/newlines)
                dict_str_clean = re.sub(r'\s+', ' ', dict_str_no_comments)
                
                # Parse the cleaned dictionary string
                ...
                print(f"[DEBUG] Dictionary parse failed: {e}")
                print(f"[DEBUG] Attempted to parse: {dict_str[:200]}...")
        
        # If we got here, extraction failed
        if command_dict is None:
            print(f"[DEBUG] No valid 'new_mpc_params' dictionary found in output")
            
    except Exception as e:
        print(f"[ERROR] Parse exception: {type(e).__name__}: {e}")
...
class RaceLLMMPC():
    ...
    def init_llm(self, model: str, model_dir: str, openai_token: str) -> tuple:
        ...
        if model == 'gpt-4o':
            ...
            if self.quant:
                ...
                # Try Mac-compatible version first, fall back to original
                try:
                    from inference.inf_pipeline_mac import RaceLLMPipelineCompat
                    llm = RaceLLMPipelineCompat(model_dir=model_dir, chat_template='qwen-2.5', load_in_4bit=True)
                except ImportError:
    ...
    def load_memory(self) -> tuple:
        ...
        if _LOCAL_EMBEDDINGS is None:
            print("[INFO] Embeddings unavailable — RAG disabled. Using base memory only.")
            return base_mem, None
        ...
        try:
            ...
            print(f"[WARN] Failed to create RAG index: {e}. Using base memory only.")
            return base_mem, None
    ...
    def load_decision_mem(self):
        ...
        try:
            ...
            print(f"[WARN] Failed to load decision memory: {e}")
            return None
    ...
    def _step_f1tenth(self, mpc_params: dict, append_history: bool = True) -> tuple:
        ...
        try:
            speed = float(np.clip(
                float(mpc_params.get('v_max', 12.0)) * 0.4,
                float(mpc_params.get('v_min', 1.0)),
                float(mpc_params.get('v_max', 12.0)),
            ))
        except (TypeError, ValueError):
            print("[WARN] Error extracting speed parameters. Using default.")
            speed = 6.0  # Default speed
    ...
    def _mpc_params_to_steering(self, params: dict) -> float:
        ...
        """
        try:
            qn     = float(params.get('qn', 20.0))
            qalpha = float(params.get('qalpha', 7.0))
            raw    = (qn - qalpha) / (qn + qalpha + 1e-6)
            return float(np.clip(raw * 0.4189, -0.4189, 0.4189))
        except (TypeError, ValueError) as e:
            print(f"[WARN] Error converting params to steering: {e}. Using defaults.")
            return 0.0  # Default: no steering
...
"""
    ...
    # ─── Output parsing (IMPROVED to handle comments in LLM outputs) ─────────

    def _sanitize_tune_output(self, text: str) -> tuple:
        """
        Extract MPC parameters from LLM output using improved parser.
        
        IMPROVEMENT: Uses llm_output_parser.parse_mpc_output() which handles:
        - XML-wrapped outputs
        - Plain dictionary outputs
        - **Dictionaries with inline comments** (common in LLM-generated code)
        
        This fixes the parsing issue where LLM outputs with comments like:
            new_mpc_params = {
                'qv': 2,  # Increase weight for velocity
                'qn': 20, # Increase lateral tracking
            }
        
        were failing with "'{' was never closed" error.
        """
        return parse_mpc_output(text)
